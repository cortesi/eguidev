...
Edev owns managed-process lifetime as well as presentation. On macOS, losing the owning launcher
terminates the complete app process group without a later cleanup command. For normal shutdown,
Edev calls the app's private `app_close` tool. An app can register `DevMcp::on_shutdown` to publish
an application-owned shutdown request from the MCP runtime thread. Without this handler, Eguidev
queues `ViewportCommand::Close` for the root viewport. Edev keeps the ownership channel open and
waits for the supervisor exit event. `[app].shutdown_grace_secs` sets the escalation deadline and
defaults to 30 seconds. `stop` and `status` report the last `graceful` or `forced` result. One-shot
commands fail if teardown is forced.
